...
import { GetExpiredVouchers } from "../../../apiServices/Seller/VoucherServices";
...
const VoucherExpired = () => {
    ...
    useEffect(() => {
        const fetchApi = async () => {
            try {
                const res = await GetExpiredVouchers(currentPage);
    ...
    const HandleSelectPage = async (pageNumber) => {
        try {
            const res = await GetExpiredVouchers(pageNumber);
            window.scrollTo({
                top: 0,
                behavior: 'smooth',
            });
