...
import { GetCanceledOrders } from "../../../apiServices/Seller/OrderSellerServices";
...
function CanceledOrder() {
    ...
    useEffect(() => {
        const fetchApi = async () => {
            try {
                const res = await GetCanceledOrders(currentPage);
    ...
    const HandleSelectPage = async (pageNumber) => {
        try {
            const res = await GetCanceledOrders(pageNumber);
            window.scrollTo({
                top: 0,
                behavior: 'smooth',
            });
