...
import { GetNotConfirmOrders } from "../../../apiServices/Seller/OrderSellerServices";
...
function OrderNotConfirm() {
    ...
    useEffect(() => {
        const fetchApi = async () => {
            try {
                const res = await GetNotConfirmOrders(currentPage);
    ...
    const HandleSelectPage = async (pageNumber) => {
        try {
            const res = await GetNotConfirmOrders(pageNumber);
            window.scrollTo({
                top: 0,
                behavior: 'smooth',
            });
