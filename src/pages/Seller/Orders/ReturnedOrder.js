...
import { GetReturnedOrders } from "../../../apiServices/Seller/OrderSellerServices";
...
function ReturnedOrder(params) {
    ...
    useEffect(() => {
        const fetchApi = async () => {
            try {
                const res = await GetReturnedOrders(currentPage);
    ...
    const HandleSelectPage = async (pageNumber) => {
        try {
            const res = await GetReturnedOrders(pageNumber);
            window.scrollTo({
                top: 0,
                behavior: 'smooth',
            });
    ...
    return (
        <section style={{ width: "90%", margin: "auto", }}>
            <h1 style={{ textAlign: "center" }}>Đơn hàng đã trả</h1>
            <section>
                {orders ? <>
