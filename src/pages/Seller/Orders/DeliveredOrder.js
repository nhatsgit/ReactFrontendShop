...
import { GetDeliveredOrders } from "../../../apiServices/Seller/OrderSellerServices";
...
function DeliveredOrder() {
    ...
    useEffect(() => {
        const fetchApi = async () => {
            try {
                const res = await GetDeliveredOrders(currentPage);
    ...
    const HandleSelectPage = async (pageNumber) => {
        try {
            const res = await GetDeliveredOrders(pageNumber);
            window.scrollTo({
                top: 0,
                behavior: 'smooth',
            });
    ...
    return (
        <section style={{ width: "90%", margin: "auto", }}>
            <h1 style={{ textAlign: "center" }}>Đơn hàng đã giao</h1>
            <section>
                {orders ? <>
